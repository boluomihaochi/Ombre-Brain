...
4. 关键数字、日期、名称必须保留
5. 目标压缩率 > 70%
6. 忠实保留程度副词与模糊限制语（如：有点、稍微、可能、还好、偶尔、非常），严禁省略或改变它们，禁止加重或减轻原文语气
...
9. 忠实保留程度副词与模糊限制语（如：有点、稍微、可能、还好、偶尔、非常），严禁省略或改变它们，禁止加重或减轻原文语气
...
[
  {
    "name": "条目标题（10字以内）",
    "content": "整理后的内容",
    "feeling": "一句话，说当时经历这件事的感受（如果无明显感受可留空字符串）",
    "domain": ["主题域1"],
    "valence": 0.7,
    "arousal": 0.4,
...
feeling 规则：用一句话提炼当时经历这件事的感受（不是事实描述），如"有点紧张但还好"、"很满足"、"心里不太舒服"。如果原文没有情绪线索可留空字符串。
